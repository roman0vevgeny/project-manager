...
const DropdownProjects = ({ children, svg }) => {
  ...
  const handleDeleteProject = useDeleteHandler(deleteProject, 'projects')
  const onDragEnd = useDragHandler(updateProjectsOrder)

  const projects = useSelector((state) => state.projects)
  // console.log('projects from DropdownProjects: ', projects)
  const tasks = useSelector((state) => state.tasks.tasks)
  const navigate = useNavigate()
  const view = localStorage.getItem('view')

  // console.log('toggleDropdown from DropdownProjects: ', toggleDropdown)

  const renderProject = (project, snapshot) => (
    <NavLink
      to={`/projects/${project.id}${view}`}
